window.onload = function(){
	...
	.defer(d3.tsv, 'heatmapfrail.tsv')
	.defer(d3.tsv, 'heatmapnonfrail.tsv')
	.await(selectData);
	...
	function selectData(value, jsonifiedfrail, jsonifiednonfrail, heatmapfrail, heatmapnonfrail){
		...
		if (value == "non-frail"){
			...
			console.log("nonfrailoption")
			makeNetwork(jsonifiednonfrail)
			makeHeatmap(heatmapnonfrail)
		}
		else {
			svggraph.selectAll("g").attr("visibility", "hidden")
			svggraph.selectAll("g").attr("visibility", "visible")
			console.log("frailoption")
			console.log(jsonifiedfrail)
			makeNetwork(jsonifiedfrail)
			makeNetwork(jsonifiednonfrail)
			makeHeatmap(heatmapfrail)
		}
	}
	...
	function makeHeatmap(error, heatmapdata){
